...
Code for `Critic Crew` is in the file `src/emergingtechnologyresearch/crews/criticCrew.py`. Its usage is in `src/emergingtechnologyresearch/flow.py`. You may follow the below steps to see `Critic Pattern` in action:
1. Set `CRITIC_AGENT` to `TRUE` in the `.env` file
2. Run the chat application using the below command on the terminal:
```bash
uv run python -m src.emergingtechnologyresearch.chat
```
3. Over the chat, ask the application to research on a specific topic. 
4. After the application generates the report, you may observe its execution trace in langfuse. You will see critic agent evaluating the output of research crew and giving feedback to it. Research crew fine tunes the report based on the feedback.
