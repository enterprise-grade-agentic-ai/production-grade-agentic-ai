...
export POOL_ID=$(aws cognito-idp create-user-pool \
 --pool-name "EmergingTechnologyResearchUserPool" \
...
export CLIENT_ID=$(aws cognito-idp create-user-pool-client \
 ...
 --client-name "EmergingTechnologyResearchClientId" \
 --no-generate-secret \
...
aws cognito-idp admin-create-user \
 --user-pool-id $POOL_ID \
 --username "agenticLearner" \
 --temporary-password "helloWorld" \
 --region us-east-1 \
