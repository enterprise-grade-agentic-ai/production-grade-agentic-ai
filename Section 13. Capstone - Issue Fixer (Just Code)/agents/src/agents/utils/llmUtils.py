import logging
import os
from crewai.hooks.decorators import before_llm_call
...
def registerLlmHooks() -> None:
    logger.info("Registering LLM hooks")
    # ── LLM call debug hooks (auto-registered by the decorators) ──────────────────
    @before_llm_call
    def adaptLLMRequestForAnthropic(context: LLMCallHookContext) -> None:
        ...
        logger.info("Last 2 messages sent to LLM: %s", context.messages[-2:])
